...
export function SubscriptionProvider({ children }: { children: ReactNode }) {
  ...
  // ✅ FIX 1: isko 'true' kar diya taaki shuruat mein hi content hide rahe
  ...
  useEffect(() => {
    ...
    if (selectedCafe && selectedCafe !== "master") {
      ...
      setVendorId(null)
      // Agar vendor ID hi nahi hai toh loading band kar do
      setLoading(false) 
    }
  }, [pathname])
  ...
  useEffect(() => {
    // ✅ FIX 2: /dashboard wali condition hata di, ab yeh har page pe check karega 
    // (kyunki upar checkSubscription func mein skipPages ki condition already lagi hai)
    if (vendorId) {
      checkSubscription()
    }
  }, [vendorId, pathname])
