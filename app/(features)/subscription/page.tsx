...
import { Alert, AlertDescription, AlertTitle } from "@/components/ui/alert";
// IMPORT: Make sure the path to your DashboardContent is correct
...
export default function SubscriptionPage() {
    ...
    const pollInterval = setInterval(async () => {
      try {
        ...
        if (data.paid && data.payment_id) {
          ...
          const activateResponse = await subscriptionApi.verifyPayment(
            vendorId,
            {
              razorpay_order_id: pollingOrderId,
              razorpay_payment_id: data.payment_id,
  ...
  return (
    <div className="relative min-h-screen w-full overflow-hidden">
      {/* 1. BACKGROUND LAYER: Blurred Dashboard */}
      {/* Changed blur-[10px] to blur-[4px] for less intense blur */}
      <div className="fixed inset-0 z-0 pointer-events-none filter blur-[-1px] scale-105 opacity-210">
        <DashboardContent activeTab="gaming-cafe" setActiveTab={() => {}} />
      </div>

      {/* 2. OVERLAY LAYER: Subscription Content */}
      <div className="fixed inset-0 z-10 bg-background/60 backdrop-blur-md overflow-y-auto">
        {/* Navigation / Close */}
            ...
            {pollingOrderId && (
              ...
              >
                <Alert className="border-primary bg-primary/5 backdrop-blur-sm max-w-2xl mx-auto">
                  ...
                  <AlertDescription>
                    We are automatically checking your payment status. Please
                    don't refresh.
                  </AlertDescription>
                </Alert>
              </motion.div>
