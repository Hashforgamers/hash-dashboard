...
export function ButtonDestructive({ isNavPinned = false }: ButtonDestructiveProps) {
  ...
  return (
    <button
      className={`group/nav flex min-h-[32px] items-center gap-2 rounded-lg border border-transparent px-2.5 py-[clamp(0.2rem,0.55vh,0.38rem)] text-sm font-medium leading-tight text-red-400 transition-all duration-200 hover:border-border/70 hover:bg-muted/50 hover:text-red-300 ${responsiveItemPaddingClass}`}
      onClick={() => {
