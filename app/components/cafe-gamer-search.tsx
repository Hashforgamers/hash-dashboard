...
export function CafeGamerSearch({cafeId, disabled, onSelect}: {cafeId:string|number; disabled:boolean; onSelect:(gamer:CafeGamer)=>void}) {
  ...
  return <div className="space-y-2">
    <label className="block text-xs font-medium">Find gamer
    ...
    {!loading&&!error&&(query.trim().length>=2||/^\d+$/.test(query.trim()))&&rows.length===0&&<p className="text-xs text-muted-foreground">No matching gamers.</p>}
