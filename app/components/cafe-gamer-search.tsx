...
export function CafeGamerSearch({cafeId, disabled, onSelect}: {cafeId:string|number; disabled:boolean; onSelect:(gamer:CafeGamer)=>void}) {
  ...
  return <div className="space-y-2">
    <label className="block text-xs font-medium">Find a Hash gamer
      <Input disabled={disabled} value={query} onChange={e=>setQuery(e.target.value)} placeholder="Name, phone, email or Hash gamer ID" autoComplete="off" />
    </label>
    {loading && <p role="status" className="text-xs text-muted-foreground">Searching…</p>}
    {error && <p role="alert" className="text-xs text-destructive">{error}</p>}
    {!loading&&!error&&query.trim().length>=2&&rows.length===0&&<p className="text-xs text-muted-foreground">No matching gamers.</p>}
    {rows.length>0&&<div className="max-h-56 overflow-auto rounded-md border"><table className="w-full text-left text-xs"><thead className="sticky top-0 bg-background"><tr><th className="p-2">Gamer</th><th className="p-2">Phone / email</th><th className="p-2">Hash ID</th><th className="p-2"><span className="sr-only">Select</span></th></tr></thead><tbody>{rows.map(row=><tr className="border-t" key={row.id}><td className="p-2 font-medium">{row.name}</td><td className="p-2"><div>{row.phone||'—'}</div><div className="text-muted-foreground">{row.email||'—'}</div></td><td className="p-2">{row.game_username||`#${row.id}`}<span className="block text-muted-foreground">#{row.id}</span></td><td className="p-2"><Button variant="outline" size="sm" disabled={disabled} onClick={()=>{onSelect(row);setQuery('');}}>Select</Button></td></tr>)}</tbody></table></div>}
  </div>;
}
