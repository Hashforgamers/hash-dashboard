      ...
      <div className="flex flex-col items-center gap-3 text-center px-6">
        ...
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1 max-w-[250px]">
            Subscribe to create and manage cafe passes.
          </p>
        </div>
        <button
...
export default function ManagePassesPage() {
  ...
  if (!hasMounted) {
    return (
      // ── relative wrapper so LockedOverlay (absolute) is scoped here ──
  ...
  const handleSubmit = async (e: any) => {
    ...
    await onSave(
      passObj.id,
      {
        ...
        days_valid: Number(form.days_valid),
        total_hours:
          passObj.pass_mode === "hour_based" ? Number(form.total_hours) : undefined,
      },
      () => setOpen(false)
    );
    setIsSubmitting(false);
